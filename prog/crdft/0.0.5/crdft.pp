...
|        -radical-attack Fukui function [f0(r)] .
...
|  -version information :
|    -changes done :
|      -added radical-attack Fukui function (but not tested) ;
|    -to do in future versions :
|      -implementing coloring a isosurface with other property .
...
|  -helpCT :
|    -command-line usage :
|      -'crdft <property> [<parameter> <value> ...]' ;
|    -command-line parameters :
|      -<rpoperty> (parameter 1) :
|        -"fukuiNu", "f-" :
|          -calculates nucleophilic Fukui function [f-(r)] from
|           _ electron densities .
|          -considered parameters :
|            -rhoN, rhoC, demonOut, scriptName, xyz ;;
|        -"fukuiEl", "f+" :
|          -calculates electriphilic Fukui function [f+(r)] from
|           _ electron densities .
|          -considered parameters :
|            -rhoN, rhoA, demonOut, scriptName, xyz ;;
|        -"fukuiRad", "f0" :
|          -calculates radical-attack Fukui function [f0(r)] from
|           _ electron densities .
|          -considered parameters :
|            -rhoC, rhoA, demonOut, scriptName, xyz ;;
|        -"dualDesc", "dual" :
...
|    -datFukuiNu :-name for output nucleophilic Fukui function data file .
|      -default value :-"fukuiNu.asc" ;;
|    -datFukuiEl :-name for output electrphilic Fukui function data file .
|      -default value :-"fukuiEl.asc" ;;
|    -datFukuiRad :-name for output radical-attack Fukui function data file .
|      -default value :-"fukuiRad.asc" ;;
...
{|-procedure crdft_init; :
 ...
 |  -looks for the external user parameter file and updates parameters .
...
{linking condText unit log file to iml}
  ...
  procedure printHelp;
    begin
      ...
      writeln;
      writeln (crdft_name + '-v.' + crdft_version + ': Plotting of ' +
        'deMon2k CRDFT molecular properties in VMD.');
      writeln;
      writeln ('  Usage: ');
      ...
      writeln ('    [3] "fukuiRad" or "f0"       ' +
                                       '( = [f+ + f-]/2 = [rhoA - rhoC]/2 )');
      writeln ('    [4] "dualDesc" or "dual"     ' +
                                       '( = f+ - f- = rhoA - 2*rhoN + rhoC )');
      writeln ('    [5] "fukuiPotNu" or "fp-"    ( = espC - espN )');
      writeln ('    [6] "fukuiPotEl" or "fp+"    ( = espN - espA )');
      writeln ('    [7] "molProp" or "scalarField"');
      writeln ('    [8] "script" or "vmdPlot"');
      writeln ('    [9] "-h", "--help", "--helpCT"');
      writeln;
      writeln ('  Optional input parameters (default value) [used by prop]:');
      writeln ('    "-demon <file>" or "-demonOut <file>" (demon.out) [1-8]');
      writeln ('    "-rhoN <ascii_file>" (rhoN.asc) [1,2,4]');
      writeln ('    "-rhoA <ascii_file>" (rhoA.asc) [2,3,4]');
      writeln ('    "-rhoC <ascii_file>" (rhoC.asc) [1,3,4]');
      writeln ('    "-espN <ascii_file>" (espN.asc) [5,6]');
      writeln ('    "-espA <ascii_file>" (espA.asc) [6]');
      writeln ('    "-espC <ascii_file>" (espC.asc) [5]');
      writeln ('    "-propFile <ascii_file>" (prop.asc) [7,8]');
      writeln ('    "-propName <string>" ("prop") [7,8]');
      writeln;
      writeln ('  Optional parameters for output:');
      writeln ('    "-prefix <string>" ("") [1-8]');
      writeln ('    "-tcl <file>", "-vmd <file>", or "-script <file>" ' +
                                                        '(vmdPlot.tcl) [1-8]');
      writeln ('    "-xyz <file>" or "-molecule <file>" (mol.xyz) [1-8]');
      writeln;
      writeln ('  Notes:');
...
{checking <function> option}
      case upCase(ParamStr(1)) of
        ...
        'FUKUIRAD', 'F0' : plotFunction := 'fukuiRad';
  ...
  procedure specifiedParam;
    begin
      case plotFunction of
        'fukuiNu','fukuiEl','fukuiRad','dualDesc','fukuiPotNu','fukuiPotEl' :
          begin
...
function fukuiRad (rhoC, rhoA : Real) : Real;
  begin
    fukuiRad := (rhoA - rhoC)/2
  end;
...
{assigning input and output files}
      if (plotFunction = 'fukuiNu') or (plotFunction = 'fukuiEl') or
         (plotFunction = 'fukuiRad') or (plotFunction = 'dualDesc') then
        begin
{for rhoN}
          if (plotFunction = 'fukuiNu') or (plotFunction = 'fukuiEl') or
             (plotFunction = 'dualDesc') then
...
{for rhoA}
          if (plotFunction = 'fukuiEl') or (plotFunction = 'fukuiRad') or
             (plotFunction = 'dualDesc') then
            begin
...
{for rhoC}
          if (plotFunction = 'fukuiNu') or (plotFunction = 'fukuiRad') or
             (plotFunction = 'dualDesc') then
            begin
              iml.infoMsg (0,2,name+'Reading rhoC data from: ' + datRhoC);
              if plotFunction = 'fukuiNu' then
                Assign (inpDat2, datRhoC)
              else if plotFunction = 'fukuiRad' then
                Assign (inpDat1, datRhoC)
              ...
              if writeInpData then
                begin
                  ...
                  if plotFunction = 'fukuiNu' then
                    ...
                    Assign (inpOutDat1, outPrefix + 'rhoC_p.asc.')
                  else   {plotFunction = dualDesc}
...
{selecting function}
      case plotFunction of
        ...
        'fukuiRad' :
          begin
            datProp := outPrefix + 'fukuiRad.asc';
            iml.infoMsg (0,2,name+'Writting Fukui-Rad data to file: '+datProp);
            crdftFunc2 := @fukuiRad
          end;
        'dualDesc' :
          begin
...
{closing files}
  ...
  procedure addTclDecl;
    begin
      ...
      script.outDev('');
      script.outDev('# cd ' + GetEnvironmentVariable('PWD'));
      script.outDev('');
